	...
	<table class="table">
		...
		<tbody>
			...
			%>
				...
				<td><a href="/ecommerce/user/searchclient?key=用户编号&value=<%=list.get(i).getCno() %>"><%=list.get(i).getCno() %></a></td>
				<td><a href="/ecommerce/user/searchshop?key=商品编号&value=<%=list.get(i).getSno() %>"><%=list.get(i).getSno() %></a></td>
				...
				<td>
					<a href="/ecommerce/user/deletebuy?bno=<%=list.get(i).getBno() %>">删除</a>&nbsp;
					<a href="/ecommerce/user/getonebuy?bno=<%=list.get(i).getBno() %>&pay=no">修改</a>
				</td>
				</tr>	
			<% } %>
		</tbody>
...
<nav aria-label="Page navigation example">
  <ul class="pagination justify-content-center">
    ...
    <% for(int j=1;j<=n;j++){ %>
    <li class="page-item"><a class="page-link" href="/ecommerce/manager/see-buy.jsp?sum=<%=j %>"><%=j %></a></li>
    <% } %>
    <li class="page-item">
    	&nbsp;&nbsp;共有<%=n %>页
    </li>
